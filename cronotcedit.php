...
// Create page object
$cronotc_edit = new ccronotc_edit();
$Page =& $cronotc_edit;

// Page init
$cronotc_edit->Page_Init();

// Page main
$cronotc_edit->Page_Main();
?>
...
// Create page object
var cronotc_edit = new ew_Page("cronotc_edit");

// page properties
cronotc_edit.PageID = "edit"; // page ID
cronotc_edit.FormID = "fcronotcedit"; // form ID
var EW_PAGE_ID = cronotc_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
cronotc_edit.ValidateForm = function(fobj) {
...
cronotc_edit.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
cronotc_edit.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
cronotc_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
cronotc_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $cronotc->TableCaption() ?><br><br>
<a href="<?php echo $cronotc->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$cronotc_edit->ShowMessage();
?>
<form name="fcronotcedit" id="fcronotcedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return cronotc_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="cronotc">
...
<?php if ($cronotc->id->Visible) { // id ?>
	<tr<?php echo $cronotc->RowAttributes() ?>>
		...
		<td<?php echo $cronotc->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $cronotc->id->ViewAttributes() ?>><?php echo $cronotc->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($cronotc->id->CurrentValue) ?>">
</span><?php echo $cronotc->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
class ccronotc_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$cronotc->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$cronotc->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$cronotc->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$cronotc->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($cronotc->id->CurrentValue == "")
			$this->Page_Terminate("cronotclist.php"); // Invalid key, return to list
			...
			Case "U": // Update
				...
				if ($this->EditRow()) { // Update record based on key
					...
					if (ew_GetPageName($sReturnUrl) == "cronotcview.php")
						$sReturnUrl = $cronotc->ViewUrl(); // View paging, return to View page directly
		...
		// Render the record
		$cronotc->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
