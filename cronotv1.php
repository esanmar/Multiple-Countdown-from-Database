...
if ($hora < $turno) {
	$sql    = "SELECT id, nombre, paquete, placa, horaini, horafin, color FROM cronotc WHERE  visible='1' and placa like '%Placa%' and  nombre like '%e%' and horaini < " . $turno;
} else {
	$sql    = "SELECT id, nombre, paquete, placa, horaini, horafin, color FROM cronotc WHERE  visible='1' and placa like '%Placa%' and  nombre like '%e%' and horaini >= " . $turno;
}
...
// Muestra Peces, Baneras y Booster. Si no tiene datos saca BBAA

if ( $result->num_rows === 0)
{
		if ($hora < $turno) {
			$sql2    = "SELECT id, nombre, paquete, placa, horaini, horafin, color FROM cronotc WHERE  visible='1' and  nombre like '%BB%' and horaini < " . $turno;
		} else {
			$sql2    = "SELECT id, nombre, paquete, placa, horaini, horafin, color FROM cronotc WHERE  visible='1' and  nombre like '%BB%' and horaini >= " . $turno;
		}
		//echo " sql2 " . $sql2;
