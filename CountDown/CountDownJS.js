...
function CountDownObject(para1) {
	...
	this.SET_YOUR_DAY = 13;
	this.SET_YOUR_MONTH = 03;
	...
	this.IS_DYNAMIC_COLOR = false; // back color will vary(true or false)

	this.CANVAS_NAME = "CountDownCanvas" ; //canvas name in html-code
		//alert(this.CANVAS_NAME);
	var
		timeAndDateLast,
		param,
		img,
	...
	function TimerEnd(){
		...
		CanvasContext.fillText("FINALIZADO", Canvas.width/2, 84.5*param.elementSize);
	}
	...
	function Main(){
		...
		if( timeAndDateNow.hours >= 1 ) {
	     backC = "#437921";
			 digitsC = "#d6dfe6";
			 this.BACK_COLOR = "#437921"; // Verde
	     this.DIGITS_COLOR = "#d6dfe6"; 
		} 
		else {
			if( timeAndDateNow.mins <= 15) {
				 this.BACK_COLOR = "#e33c4f"; // Rojo
		     this.DIGITS_COLOR = "#d6dfe6"; 
		     backC = "#e33c4f";
				 digitsC = "#d6dfe6";
				} else {
					  this.DIGITS_COLOR = "#f07000"; // Naranja
	...
	function resizeCanvas(){
		...
		param = new ParametresType();
		//edsa01 height = 150.1*param.elementSize + 1;
	...
	this.Start = function(){
		...
		//edsa01 document.write('<div style="top: 0px;">');
		document.write('<img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" id="ResizingImage' + this.CANVAS_NAME  + para1 + '" width="100%" height="1"></img>');
		document.write('<canvas id="' + this.CANVAS_NAME + para1 + '"></canvas>');
		//edsa01 document.write('</div>');
		
		findWidth = document.getElementById('ResizingImage' + this.CANVAS_NAME + para1);
		width = findWidth.width;

		Canvas = document.getElementById(this.CANVAS_NAME + para1);
