...
if ($hora < $turno) {
	$sql    = "SELECT id, nombre, paquete, placa, horaini, horafin, color FROM cronotc WHERE visible='1' and (nombre like '%14 G%' or nombre like '%15%' or nombre like '%23%') and horaini < " . $turno;
} else {
	$sql    = "SELECT id, nombre, paquete, placa, horaini, horafin, color FROM cronotc WHERE visible='1' and (nombre like '%14 G%' or nombre like '%15%' or nombre like '%23%') and horaini >= " . $turno;
}
...
// Muestra 2.15, 2.14 G, 2.23 . Si no tiene datos saca 2.09/TRICORNIOS  

if ( $result->num_rows === 0)
{
		if ($hora < $turno) {
			$sql2    = "SELECT id, nombre, paquete, placa, horaini, horafin, color FROM cronotc WHERE  visible='1' and placa like '%Placa%' and (nombre like '%09%' or nombre like '%tri%') and horaini < " . $turno;
		} else {
			$sql2    = "SELECT id, nombre, paquete, placa, horaini, horafin, color FROM cronotc WHERE  visible='1' and placa like '%Placa%' and (nombre like '%09%' or nombre like '%tri%') and horaini >= " . $turno;
...
?>
						<tr>
							...
							<td align="center"><span  style="text-Shadow: 0px 1px 2px black; font-size: 50px; font-weight: bold; color: #ffffff; background-color: <?php echo $color;?>"> <?php echo $placa;?></span></td>
