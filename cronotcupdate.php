...
// Create page object
$cronotc_update = new ccronotc_update();
$Page =& $cronotc_update;

// Page init
$cronotc_update->Page_Init();

// Page main
$cronotc_update->Page_Main();
?>
...
// Create page object
var cronotc_update = new ew_Page("cronotc_update");

// page properties
cronotc_update.PageID = "update"; // page ID
cronotc_update.FormID = "fcronotcupdate"; // form ID
var EW_PAGE_ID = cronotc_update.PageID; // for backward compatibility

// extend page with ValidateForm function
cronotc_update.ValidateForm = function(fobj) {
...
cronotc_update.Form_CustomValidate =  
 function(fobj) { // DO NOT CHANGE THIS LINE!

 	// Your custom validation code here, return false if invalid. 
 	return true;
 }
cronotc_update.SelectAllKey = function(elem) {
	ew_SelectAll(elem);
	ew_ClickAll(elem);
}
<?php if (EW_CLIENT_VALIDATE) { ?>
cronotc_update.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
cronotc_update.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Update") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $cronotc->TableCaption() ?><br><br>
<a href="<?php echo $cronotc->getReturnUrl() ?>"><?php echo $Language->Phrase("BackToList") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$cronotc_update->ShowMessage();
?>
<form name="fcronotcupdate" id="fcronotcupdate" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return cronotc_update.ValidateForm(this);">
<p>
<input type="hidden" name="t" id="t" value="cronotc">
<input type="hidden" name="a_update" id="a_update" value="U">
<?php for ($i = 0; $i < $cronotc_update->nKeySelected; $i++) { ?>
<input type="hidden" name="k<?php echo $i+1 ?>_key" id="key<?php echo $i+1 ?>" value="<?php echo ew_HtmlEncode($cronotc_update->arRecKeys[$i]) ?>">
<?php } ?>
...
<input type="checkbox" name="u_nombre" id="u_nombre" value="1"<?php echo ($cronotc->nombre->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $cronotc->nombre->CellAttributes() ?>><?php echo $cronotc->nombre->FldCaption() ?></td>
...
<input type="checkbox" name="u_horaini" id="u_horaini" value="1"<?php echo ($cronotc->horaini->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $cronotc->horaini->CellAttributes() ?>><?php echo $cronotc->horaini->FldCaption() ?></td>
...
<input type="checkbox" name="u_horafin" id="u_horafin" value="1"<?php echo ($cronotc->horafin->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $cronotc->horafin->CellAttributes() ?>><?php echo $cronotc->horafin->FldCaption() ?></td>
...
<input type="checkbox" name="u_visible" id="u_visible" value="1"<?php echo ($cronotc->visible->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $cronotc->visible->CellAttributes() ?>><?php echo $cronotc->visible->FldCaption() ?></td>
...
<input type="checkbox" name="u_paquete" id="u_paquete" value="1"<?php echo ($cronotc->paquete->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $cronotc->paquete->CellAttributes() ?>><?php echo $cronotc->paquete->FldCaption() ?></td>
...
<input type="checkbox" name="u_placa" id="u_placa" value="1"<?php echo ($cronotc->placa->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $cronotc->placa->CellAttributes() ?>><?php echo $cronotc->placa->FldCaption() ?></td>
...
<input type="checkbox" name="u_color" id="u_color" value="1"<?php echo ($cronotc->color->MultiUpdate == "1") ? " checked=\"checked\"" : "" ?>>
</td>
		<td<?php echo $cronotc->color->CellAttributes() ?>><?php echo $cronotc->color->FldCaption() ?></td>
...
class ccronotc_update {
	...
	// Page object name
	var $PageObjName = 'cronotc_update';
	...
	function Page_Main() {
		...
		if ($this->nKeySelected == 0) {
			...
			if (@$_POST["a_update"] <> "") {
				...
				// Get action
				$cronotc->CurrentAction = $_POST["a_update"];
		...
		if ($this->nKeySelected <= 0)
			$this->Page_Terminate("cronotclist.php"); // No records selected, return to list
		...
		if ($rs = $this->LoadRecordset()) {
			...
			while (!$rs->EOF) {
				if ($i == 1) {
					...
					if (!ew_CompareValue($cronotc->nombre->DbValue, $rs->fields('nombre')))
						$cronotc->nombre->CurrentValue = NULL;
					if (!ew_CompareValue($cronotc->horaini->DbValue, $rs->fields('horaini')))
						$cronotc->horaini->CurrentValue = NULL;
					if (!ew_CompareValue($cronotc->horafin->DbValue, $rs->fields('horafin')))
						$cronotc->horafin->CurrentValue = NULL;
					if (!ew_CompareValue($cronotc->visible->DbValue, $rs->fields('visible')))
						$cronotc->visible->CurrentValue = NULL;
					if (!ew_CompareValue($cronotc->paquete->DbValue, $rs->fields('paquete')))
						$cronotc->paquete->CurrentValue = NULL;
					if (!ew_CompareValue($cronotc->placa->DbValue, $rs->fields('placa')))
						$cronotc->placa->CurrentValue = NULL;
					if (!ew_CompareValue($cronotc->color->DbValue, $rs->fields('color')))
						$cronotc->color->CurrentValue = NULL;
				}
				$i++;
				$rs->MoveNext();
			}
	...
	function SetupKeyValues($key) {
		...
		if (!is_numeric($sKeyFld))
			return FALSE;
		$cronotc->id->CurrentValue = $sKeyFld;
		return TRUE;
	}

	// Update all selected rows
	function UpdateRows() {
		global $conn, $Language, $cronotc;
		$conn->BeginTrans();
	...
	function LoadFormValues() {
		...
		$cronotc->nombre->MultiUpdate = $objForm->GetValue("u_nombre");
		$cronotc->horaini->setFormValue($objForm->GetValue("x_horaini"));
		$cronotc->horaini->MultiUpdate = $objForm->GetValue("u_horaini");
		$cronotc->horafin->setFormValue($objForm->GetValue("x_horafin"));
		$cronotc->horafin->MultiUpdate = $objForm->GetValue("u_horafin");
		$cronotc->visible->setFormValue($objForm->GetValue("x_visible"));
		$cronotc->visible->MultiUpdate = $objForm->GetValue("u_visible");
		$cronotc->paquete->setFormValue($objForm->GetValue("x_paquete"));
		$cronotc->paquete->MultiUpdate = $objForm->GetValue("u_paquete");
		$cronotc->placa->setFormValue($objForm->GetValue("x_placa"));
		$cronotc->placa->MultiUpdate = $objForm->GetValue("u_placa");
		$cronotc->color->setFormValue($objForm->GetValue("x_color"));
		$cronotc->color->MultiUpdate = $objForm->GetValue("u_color");
	...
	function ValidateForm() {
		...
		$gsFormError = "";
		$lUpdateCnt = 0;
		if ($cronotc->nombre->MultiUpdate == "1") $lUpdateCnt++;
		if ($cronotc->horaini->MultiUpdate == "1") $lUpdateCnt++;
		if ($cronotc->horafin->MultiUpdate == "1") $lUpdateCnt++;
		if ($cronotc->visible->MultiUpdate == "1") $lUpdateCnt++;
		if ($cronotc->paquete->MultiUpdate == "1") $lUpdateCnt++;
		if ($cronotc->placa->MultiUpdate == "1") $lUpdateCnt++;
		if ($cronotc->color->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
